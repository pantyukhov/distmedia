...
@deconstructible
class DistrubitedFileSystemStorage(Storage, ABC):
    ...
    def __init__(self, api_url=None, gateway_url=None):
        ...
        """
        self._api_address = api_url or DEFAULT_ADDR
        self.__ipfshttpclient = None
        # self.gateway_url = gateway_url or getattr(settings, 'IPFS_STORAGE_GATEWAY_URL', 'https://ipfs.io/ipfs/')

    @property
    def _ipfshttpclient(self):
        if self.__ipfshttpclient is None:
            self.__ipfshttpclient = ipfshttpclient.connect(
                self._api_address,
            )
    ...
    def _open(self, name: str, mode='rb') -> File:
        ...
        """
        return ContentFile(self._ipfshttpclient.cat(name), name=name)
    ...
    def _save(self, name: str, content: File) -> str:
        ...
        """
        multihash = self._ipfshttpclient.add_bytes(content.__iter__())
        self._ipfshttpclient.pin_add(multihash)
        return multihash
    ...
    def size(self, name: str) -> int:
        ...
        return self._ipfshttpclient.object_stat(name)['CumulativeSize']

    def delete(self, name: str):
        """Unpin IPFS content from the daemon."""
        self._ipfshttpclient.pin_rm(name)
